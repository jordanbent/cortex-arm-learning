...
const mi_parse_1 = require("../src/backend/mi_parse");
suite("MI Parse", () => {
    test("Simple out of band record", () => {
        let parsed = mi_parse_1.parseMI(`4=thread-exited,id="3",group-id="i1"`);
        assert.ok(parsed);
    ...
    test("Console stream output with new line", () => {
        let parsed = mi_parse_1.parseMI(`~"[Thread 0x7fffe993a700 (LWP 11002) exited]\\n"`);
        assert.ok(parsed);
    ...
    test("'(gdb)' line", () => {
        let parsed = mi_parse_1.parseMI(`(gdb)`);
        assert.ok(parsed);
        assert.equal(parsed.token, undefined);
        assert.equal(parsed.outOfBandRecord.length, 0);
        assert.equal(parsed.resultRecords, undefined);
    ...
    test("Simple result record", () => {
        let parsed = mi_parse_1.parseMI(`1^running`);
        assert.ok(parsed);
    ...
    test("Advanced out of band record (Breakpoint hit)", () => {
        let parsed = mi_parse_1.parseMI(`*stopped,reason="breakpoint-hit",disp="keep",bkptno="1",frame={addr="0x00000000004e807f",func="D main",args=[{name="args",value="..."}],file="source/app.d",fullname="/path/to/source/app.d",line="157"},thread-id="1",stopped-threads="all",core="0"`);
        assert.ok(parsed);
        assert.equal(parsed.token, undefined);
        assert.equal(parsed.outOfBandRecord.length, 1);
        assert.equal(parsed.outOfBandRecord[0].isStream, false);
    ...
    test("Advanced result record", () => {
        let parsed = mi_parse_1.parseMI(`2^done,asm_insns=[src_and_asm_line={line="134",file="source/app.d",fullname="/path/to/source/app.d",line_asm_insn=[{address="0x00000000004e7da4",func-name="_Dmain",offset="0",inst="push   %rbp"},{address="0x00000000004e7da5",func-name="_Dmain",offset="1",inst="mov    %rsp,%rbp"}]}]`);
        assert.ok(parsed);
