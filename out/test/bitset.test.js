...
const fixedbitset_1 = require("../src/frontend/fixedbitset");
const addrranges_1 = require("../src/frontend/addrranges");
fixedbitset_1.FixedBitSet.doAsserts = true;
suite('FixedBitSet and Address Range Tests', () => {
    test('FixedBitSet creation set/clr/inv', () => {
        const bs = new fixedbitset_1.FixedBitSet(1);
    ...
    test('FixedBitSet creation set/clr/inv/itor. various sizes', () => {
        for (let sz = 0; sz < 200; sz++) {
            const bs = new fixedbitset_1.FixedBitSet(sz);
            if (sz > 1) {
                const last = sz - 1;
                const mid = sz >>> 1;
                bs.setBit(last);
    ...
    test('AddressRangesInUse creation/methods. various sizes', () => {
        const len = 64;
        const bs = new addrranges_1.AddressRangesInUse(len);
        for (let ix = 0; ix < (len - 4); ix += 4) { // 4 bits represents a word
            ...
            if (ix >= 16) {
                ...
                assert.deepStrictEqual(newRanges1[0], new addrranges_1.AddrRange(16 + 8, 4), 'ranges are not equal');
                ...
                for (let iter = 0; iter < 2; iter++) {
                    ...
                    if (ix < 32) {
                        ...
                        assert.deepStrictEqual(newRanges3[0], new addrranges_1.AddrRange(16 + 8, 16 + (ix - 16)), 'ranges are not equal');
                    }
                    else {
    ...
    test('AddressRangesInUse alignment tests', () => {
        for (let alignIter = 0; alignIter < 2; alignIter++) {
            ...
            assert.deepStrictEqual(ranges1, [new addrranges_1.AddrRange(16 + 8, isAligned ? 4 : 3)]);
            ...
            assert.deepStrictEqual(ranges2[1], new addrranges_1.AddrRange(16 + 23 - (isAligned ? 3 : 0), isAligned ? 8 : 5));
            if (isAligned) {
                continue;
            }
            ...
            const mergedRange = [
                new addrranges_1.AddrRange(ranges2[0].base, ranges2[1].nxtAddr() - ranges2[0].base)
            ];
            for (; gap < len; gap++) {
                const ranges3 = bs.getAddressRangesOptimized(16, isAligned, gap);
...
//# sourceMappingURL=bitset.test.js.map