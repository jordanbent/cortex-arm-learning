...
const fs = require("fs");
const os = require("os");
const OPENOCD_VALID_RTOS = ['eCos', 'ThreadX', 'FreeRTOS', 'ChibiOS', 'embKernel', 'mqx', 'uCOS-III', 'auto'];
const JLINK_VALID_RTOS = ['FreeRTOS', 'embOS'];
class CortexDebugConfigurationProvider {
    constructor(context) {
    ...
    resolveDebugConfiguration(folder, config, token) {
        ...
        if (!config.preRestartCommands) {
            config.preRestartCommands = [];
        }
        if (!config.postRestartCommands) {
            config.postRestartCommands = [];
        }
        if (config.request !== 'launch') {
            config.runToMain = false;
        }
        switch (type) {
            case 'jlink':
    ...
    verifyQEMUConfiguration(folder, config) {
        if (config.qemupath && !config.serverpath) {
            config.serverpath = config.qemupath;
        }
        if (!config.cpu) {
    ...
    verifyJLinkConfiguration(folder, config) {
    ...
    verifyOpenOCDConfiguration(folder, config) {
    ...
    verifySTUtilConfiguration(folder, config) {
    ...
    verifyPyOCDConfiguration(folder, config) {
    ...
    verifyBMPConfiguration(folder, config) {
    ...
    verifyExternalConfiguration(folder, config) {
        if (config.swoConfig.enabled && config.swoConfig.source === 'probe') {
...
exports.CortexDebugConfigurationProvider = CortexDebugConfigurationProvider;
