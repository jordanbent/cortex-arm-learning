...
class TimeseriesGraph {
    constructor(configuration, datasource) {
        ...
        this.line = d3.line().x((d) => this.x(d.timestamp)).y((d) => this.y(d.value));
        ...
        this.sline = d3.line().x((d) => this.sx(d.timestamp)).y((d) => this.sy(d.value));
    ...
    updateGraph() {
        if (!this.stopped) {
            ...
            const lines = this.ag.selectAll('line.annotation').data(visAnnotations).attr('x1', (d) => this.x(d.timestamp)).attr('x2', (d) => this.x(d.timestamp));
            // tslint:disable-next-line:max-line-length
            lines.enter().append('line').classed('annotation', true).attr('stroke', (d) => d.type === 'continued' ? 'rgba(0, 255, 0, 0.25)' : 'rgba(255, 0, 0, 0.25)').attr('stroke-width', 1).attr('y1', 0).attr('y2', this.height).attr('x1', (d) => this.x(d.timestamp)).attr('x2', (d) => this.x(d.timestamp));
            lines.exit().remove();
            ...
            this.spaths.forEach((path) => {
                ...
                if (oldest && oldest.timestamp < startTime) {
...
//# sourceMappingURL=timeseriesgraph.js.map